document.getElementById('signatureForm').addEventListener('submit', function(event) {
    ...
    let selectedIconColor = ((currentCoopColorCheckbox === true) ? coopColor : "currentColor");
    let selectedUrlColor = ((currentUrlColorCheckbox === true) ? linkColor : "currentColor");
     ...
     const loadDesign = (design) => {
        fetch(`${design}.html`)
            ...
            .then(html => {
                ...
                if (email) signature.querySelector('#email').innerHTML = `${emailIcon} <a style="color:${selectedUrlColor}" href="mailto:${email}">${email}</a>`;
                if (phone) signature.querySelector('#phone').innerHTML = `${phoneIcon} <a style="color:${selectedUrlColor}" href="tel:${phone}">${phone}</a>`;
                if (mastodonUrl) signature.querySelector('#mastodon').innerHTML = `${mastodonIcon} <a style="color:${selectedUrlColor}" href="${mastodonUrl}">${mastodonUser}</a>`;

                if (coop) signature.querySelector('#coop').textContent = coop;
                if (coopPhone) signature.querySelector('#coopPhone').innerHTML = `${phoneIcon} <a style="color:${selectedUrlColor}" href="tel:${coopPhone}">${coopPhone}</a>`;
                if (website) signature.querySelector('#website').innerHTML = `${websiteIcon} <a style="color:${selectedUrlColor}" href="${website}" target="_blank">${website}</a>`;
                if (coopEmail) signature.querySelector('#coopEmail').innerHTML = `${emailIcon} <a style="color:${selectedUrlColor}" href="mailto:${coopEmail}">${coopEmail}</a>`;
                if (coopMastodonUrl) signature.querySelector('#coopMastodon').innerHTML = `${mastodonIcon} <a style="color:${selectedUrlColor}" href="${coopMastodonUrl}">${coopMastodonUser}</a>`;
