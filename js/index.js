document.addEventListener("DOMContentLoaded", function () {
    ...
    const handleSVGFile = async (file) => {
        if (!file || !file.name.endsWith(".svg")) {
            alert("Mesedez SVG fitxategi bat igo");
            return;
        }

        const text = await file.text();
        uploadedSVG = processSVG(text);

        dropZone.innerHTML = `
            <p>${file.name} ✅</p>
            <div style="margin-top:10px;">${uploadedSVG}</div>
        `;
    };
    ...
    if (dropZone && fileInput) {
        ...
        fileInput.addEventListener("change", async (e) => {
            await handleSVGFile(e.target.files[0]);
        });
        ...
        dropZone.addEventListener("drop", async (e) => {
            e.preventDefault();
            dropZone.classList.remove("dragover");
            await handleSVGFile(e.dataTransfer.files[0]);
        });
    }
    ...
    form.addEventListener('submit', function(event) {
        ...
        fetch(`${design}.html`)
            ...
            .then(html => {
                ...
                if (!signature) throw new Error("Template missing .signature-design");

                const setText = (id, value) => {
                    const el = signature.querySelector(`#${id}`);
                ...
                if (email) setHTML('email', `${emailIcon} <a style="color:${selectedUrlColor}" href="mailto:${email}">${email}</a>`);
                if (phone) setHTML('phone', `${phoneIcon} <a style="color:${selectedUrlColor}" href="tel:${phone}">${phone}</a>`);
                if (mastodonUrl) setHTML('mastodon', `${mastodonIcon} <a style="color:${selectedUrlColor}" href="${mastodonUrl}">${mastodonUser}</a>`);

                setText('coop', coop);

                if (coopPhone) setHTML('coopPhone', `${phoneIcon} <a style="color:${selectedUrlColor}" href="tel:${coopPhone}">${coopPhone}</a>`);
                if (website) setHTML('website', `${websiteIcon} <a style="color:${selectedUrlColor}" href="${website}" target="_blank">${website}</a>`);
                if (coopEmail) setHTML('coopEmail', `${emailIcon} <a style="color:${selectedUrlColor}" href="mailto:${coopEmail}">${coopEmail}</a>`);
                if (coopMastodonUrl) setHTML('coopMastodon', `${mastodonIcon} <a style="color:${selectedUrlColor}" href="${coopMastodonUrl}">${coopMastodonUser}</a>`);

                // SVG injection (NEW)
                if (uploadedSVG) {
                    setHTML('logoSvg', uploadedSVG);
                }

                if (legalText) setHTML('legalText', legalText);
    ...
    copyButton.addEventListener('click', function() {
        ...
        navigator.clipboard.writeText(htmlOutput)
            .then(() => alert('HTMLa arbelera kopiatu da'))
            .catch(err => console.error('Arazoa kopiatzean:', err));
    });
